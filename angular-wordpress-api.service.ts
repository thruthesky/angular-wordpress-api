...
const wordpressUrl = 'http://work.org/test/wordpress';
const restApiUrl = wordpressUrl + '/wp-json/wp/v2';
...
export class AngularWordpressApiService {
  ...
  register(user: UserCreate) {
    return this.http.post<UserResponse>(usersUrl, user).pipe(
  ...
  updateProfile(user: UserUpdate) {
    ...
    return this.http.post<UserResponse>(usersUrl + '/me', user, options).pipe(
      tap(data => this.saveUserData(data))
    );
  }
  /**
   ...
   */
  profile() {
    return this.http.get<UserResponse>(wordpressUrl + '/wp-json/custom/api/profile', this.loginAuth).pipe(
      tap(data => this.saveUserData(data))
    );
  }
  ...
  postCreate(post: PostCreate) {
    return this.http.post<any>(postsUrl, post, this.loginAuth);
  }
